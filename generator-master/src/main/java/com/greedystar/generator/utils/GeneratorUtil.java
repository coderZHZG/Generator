...
public class GeneratorUtil {
    ...
    public static String converTableName(String tableName){
        ...
        if(tableName.indexOf("_")>0){
            String[] tabNames=tableName.split("_");
            for(String tbNa:tabNames){
                sbName.append(tbNa.substring(0,1));
            }
        }else{
            sbName.append(tableName.substring(0,3));
        }
    ...
    public static String generateMapperColumnMap(String tableName, List<ColumnInfo> infos) {
        ...
        for (int i = 0; i < infos.size(); i++) {
            ...
            String sbName=converTableName(tableName);
            sb.append(sbName).append(".").append(infos.get(i).getColumnName()).append(" AS ").append("\"").append(infos.get(i).getPropertyName()).append("\",\n");
        }
     ...
     * @return
     */
    public static String generateListParams(List<ColumnInfo> infos,String tableShortName){
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < infos.size(); i++) {
            if (i != 0) {
                sb.append("        ");
            ...
            sb.append("            ");
            sb.append("and ").append(tableShortName).append(".`").append(infos.get(i).getColumnName()).append("` = ");
     ...
     * @return
     */
    public static String generateMapperInsertBatchProperties(List<ColumnInfo> infos) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < infos.size(); i++) {
            if (i != 0) {
                sb.append("        ");
        ...
        String resStr=sb.toString().substring(1);

        return resStr;//.substring(0, sb.toString().length() - 2);
    }
    /**
     ...
     */
    public static String generateMapperInsertBatchValues(List<ColumnInfo> infos,String tableName) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < infos.size(); i++) {
            if (i != 0) {
                sb.append("        ");
            }
            sb.append("#{").append(tableName).append(".").append(infos.get(i).getPropertyName()).append("},\n");
